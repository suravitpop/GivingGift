...
<?php
	...
	if(is_home()) {
		...
		global $post;
		if(is_object($post)) {
			$scalia_page_data = get_post_meta($post->ID, 'scalia_page_data', false);
			$effects_disabled = isset($scalia_page_data['effects_disabled']) ? (bool) $scalia_page_data['effects_disabled'] : true;
		}
	}
	if($effects_disabled)
